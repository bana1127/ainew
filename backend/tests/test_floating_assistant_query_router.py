...
def test_routes_activity_count_question():
    assert route_floating_assistant_intent("활동 몇 개 있어?") == "activity_count"


def test_routes_activity_participant_question_with_named_activity():
    assert route_floating_assistant_intent("향수 활동에 몇 명 참여했어?") == "activity_participant_count"


def test_routes_membership_fee_status_question():
    assert route_floating_assistant_intent("이번 학기 회비 미납 몇 명이야?") == "membership_fee_status"


def test_routes_activity_fee_status_question():
    assert route_floating_assistant_intent("활동비 미납 있는 활동 알려줘") == "activity_fee_status"
